...
import net.diverginglensestudios.undeadremains.loot.LootTableHelper;
import net.diverginglensestudios.undeadremains.loot.ModLootModifier;
import net.diverginglensestudios.undeadremains.loot.RNGHelper;
...
public class AddItemModifier extends ModLootModifier implements IConditional {
    protected static <T extends AddItemModifier> Products.P5<RecordCodecBuilder.Mu<T>, LootItemCondition[], Item, Float, Integer, Optional<CompoundTag>> addItemCodecStart(
    ...
    public static final Codec<AddItemModifier> CODEC = RecordCodecBuilder.create(inst -> addItemCodecStart(inst)
            .apply(inst, (conditionsIn, item1, chance1, maxAmount1, tagOptional) -> new AddItemModifier(conditionsIn,
                    item1, chance1, maxAmount1, tagOptional.orElse(null))));
    final Item item;
    final int maxAmount;
    float chance;
    @Nullable
    ...
    public void addItem(Consumer<ItemStack> consumer, LootContext context, float chance) {
        ItemStack stack = RNGHelper.calculateAndDontDrop(item, maxAmount, LootTableHelper.getLivingSource(context),
                chance);
        if (tag != null)
            stack.setTag(tag);
