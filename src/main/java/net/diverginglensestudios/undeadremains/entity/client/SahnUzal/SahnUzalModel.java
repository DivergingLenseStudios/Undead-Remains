...
public class SahnUzalModel<T extends Entity> extends HierarchicalModel<T> {
	...
	public SahnUzalModel(ModelPart root) {
		...
		this.cube = this.sahn_uzal.getChild("body").getChild("torso_1").getChild("Torso_2").getChild("arm_right").getChild("Lower_arm_right").getChild("weapon").getChild("blades").getChild("cube");
	}
	...
	public static LayerDefinition createBodyLayer() {
		...
		PartDefinition leg_left = pelvis.addOrReplaceChild("leg_left", CubeListBuilder.create().texOffs(50, 105).mirror().addBox(-5.0F, 0.0F, -4.0F, 8.0F, 32.0F, 8.0F, new CubeDeformation(0.0F)).mirror(false), PartPose.offset(6.0F, 11.0F, 0.0F));
	...
	this.root().getAllParts().forEach(ModelPart::resetPose);
		...
		this.animate(((SahnUzalEntity) entity).beamAnimationState, SahnUzalAnimations.SAHN_UZAL_LAZER_ATTACK, ageInTicks, 1f);
