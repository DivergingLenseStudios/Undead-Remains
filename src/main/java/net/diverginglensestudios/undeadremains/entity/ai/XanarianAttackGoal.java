...
public class XanarianAttackGoal extends MeleeAttackGoal {
	private final XanarianEntity entity;


	public XanarianAttackGoal(PathfinderMob pMob, double pSpeedModifier, boolean pFollowingTargetEvenIfNotSeen) {
		super(pMob, pSpeedModifier, pFollowingTargetEvenIfNotSeen);
		entity = ((XanarianEntity) pMob);
	}
	...
	protected void checkAndPerformAttack(LivingEntity pEnemy, double pDistToEnemySqr) {
		if (isEnemyWithinAttackDistance(pEnemy, pDistToEnemySqr)) { //if the enemy is in reach
			...
			if(isTimeToStartAttackAnimation()){ //if both tickers are at 0
				entity.setAttackTicker(15);//length of animation (this starts the anim.)
			}
	...
	protected boolean isTimeToStartAttackAnimation() {
		return entity.getAttackTicker()<=0; //new animation can only start if the others are over
	}
