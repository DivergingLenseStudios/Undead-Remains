...
public void destroy(LevelAccessor pLevel, BlockPos pPos, BlockState pState) {
    ...
    if (pLevel instanceof ServerLevel serverLevel) {
        ...
        int choice = random.nextInt(6) + 1; // Number of events
        if (choice==1){//////////EVENT_1//////////
            SupremeZombieEntity customEntity = new SupremeZombieEntity(ModEntities.SUPREME_ZOMBIE.get(), serverLevel);
            ...
            FossilizedZombieEntity customEntity = new FossilizedZombieEntity(ModEntities.FOSSILIZED_ZOMBIE.get(), serverLevel);
            customEntity.moveTo(pPos.getX() + 0.5, pPos.getY(), pPos.getZ() + 0.5, 0, 0);
            serverLevel.addFreshEntity(customEntity);
            mobcount ++;
        }
        }else if (choice==6) {//////////EVENT_6//////////
            serverLevel.levelEvent(2001, pPos, Block.getId(ModBlocks.FOSSIL_BLOCK.get().defaultBlockState())); // Particle effect
