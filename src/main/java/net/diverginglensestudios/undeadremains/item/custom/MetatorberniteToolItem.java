...
public class MetatorberniteToolItem extends PickaxeItem {
    ...
    public MetatorberniteToolItem(Tier tier, int attackDamageModifier, float attackSpeedModifier,
    ...
    public InteractionResult useOn(UseOnContext context) {
        ...
        // Only act on top face and when space above is free (same behavior as vanilla
        // hoes/shovels)
        ...
        // SHOVEL behavior (make path) — only from grass block to avoid conflict with
        // dirt which should be tilled
    ...
    private boolean canTill(BlockState state) {
        // Tillable: grass block, dirt, coarse dirt (coarse dirt can't be tilled in
        // vanilla but adjust if desired)
        return state.is(Blocks.GRASS_BLOCK) || state.is(Blocks.DIRT) || state.is(Blocks.COARSE_DIRT);
    }

    private boolean canMakePath(BlockState state) {
        // Only create a path from grass block to avoid conflicting with dirt which
        // should be tilled
